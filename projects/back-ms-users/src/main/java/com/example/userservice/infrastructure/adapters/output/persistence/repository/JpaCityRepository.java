 ...
 */
@Repository
public interface JpaCityRepository extends JpaRepository<CityDbo, UUID> {
     ...
     */
    @Query("SELECT COUNT(e) FROM CityDbo e WHERE " +
           "(:search IS NULL OR :search = '' OR " +
           "LOWER(e.username) LIKE LOWER(CONCAT('%', :search, '%')) OR " +
           "LOWER(e.email) LIKE LOWER(CONCAT('%', :search, '%')) OR " +
           "LOWER(e.firstName) LIKE LOWER(CONCAT('%', :search, '%')) OR " +
    ...
    @Query("SELECT e FROM CityDbo e WHERE " +
           ...
           "ORDER BY e.createdAt DESC")
    Page<CityDbo> findByFilters(@Param("search") String search, 
     ...
     */
    @Query("SELECT e FROM CityDbo e ORDER BY e.createdAt DESC")
