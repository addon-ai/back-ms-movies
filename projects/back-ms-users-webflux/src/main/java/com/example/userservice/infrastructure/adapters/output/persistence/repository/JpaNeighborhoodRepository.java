...
public interface JpaNeighborhoodRepository extends R2dbcRepository<NeighborhoodDbo, UUID> {
     ...
     * 
     */
    @Query("SELECT * FROM neighborhoods u WHERE " +
     ...
     */
    @Query("SELECT COUNT(*) FROM neighborhoods u WHERE " +
           "(:search IS NULL OR :search = '' OR " +
           "LOWER(e.name) LIKE LOWER(CONCAT('%', :search, '%')) OR LOWER(e.status) LIKE LOWER(CONCAT('%', :search, '%')))")
     ...
     */
    @Query("SELECT * FROM neighborhoods u ORDER BY u.created_at DESC LIMIT :limit OFFSET :offset")
