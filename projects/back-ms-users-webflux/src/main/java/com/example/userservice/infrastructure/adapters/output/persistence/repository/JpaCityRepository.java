 ...
 */
@Repository
public interface JpaCityRepository extends R2dbcRepository<CityDbo, UUID> {
     ...
     * 
     */
    @Query("SELECT * FROM cities u WHERE " +
     ...
     */
    @Query("SELECT COUNT(*) FROM cities u WHERE " +
           "(:search IS NULL OR :search = '' OR " +
           "LOWER(e.name) LIKE LOWER(CONCAT('%', :search, '%')) OR LOWER(e.status) LIKE LOWER(CONCAT('%', :search, '%')))")
     ...
     */
    @Query("SELECT * FROM cities u ORDER BY u.created_at DESC LIMIT :limit OFFSET :offset")
